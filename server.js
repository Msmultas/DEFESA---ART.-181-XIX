...
app.post('/gerar-pdf', (req, res) => {
  ...
  doc.fontSize(14).font('Helvetica').text('oportunidade que foi autuado pelo suposto cometimento da infração prevista no Art. 1811 XIX do Código de Trânsito Brasileiro.', { align: 'left' });
  doc.moveDown();
  ...
  doc.fontSize(15).font('Helvetica-Bold').text('IV - DO MÉRITO E DO DIREITO', { align: 'center' });
  doc.moveDown();

  doc.fontSize(15).font('Helvetica-Bold').text('IV.I DA AUSÊNCIA DE SINALIZAÇÃO DO LOCAL - IMPOSSIBILIDADE DE APLICAÇÃO DA MULTA', { align: 'left' });
  doc.moveDown();

  doc.fontSize(14).font('Helvetica').text('Sabe-se que a ciência inequívoca dos condutores quanto às advertências de impossibilidade de parada ou estacionamento na via através da sinalização é condição obrigatória para justificar eventual aplicação de multa,  nos termos do Art. 90, § 1º do CTB, “in verbis”;', { align: 'left' });
  doc.moveDown();
  doc.fontSize(12).font('Helvetica').text('Art. 90. Não serão aplicadas as sanções previstas neste Código por inobservância à sinalização quando esta for insuficiente ou incorreta.', { align: 'right' });
  doc.moveDown();
  doc.fontSize(12).font('Helvetica').text('§ 1º O órgão ou entidade de trânsito com circunscrição sobre a via é responsável pela implantação da sinalização, respondendo pela sua falta, insuficiência ou incorreta colocação.', { align: 'right' });
  doc.moveDown();
  doc.fontSize(14).font('Helvetica').text('No caso em tela, o órgão responsável não cumpriu com a obrigação de sinalizar corretamente o local, o que impossibilitou que este(a) condutor(a) pudesse identificar que estaria cometendo eventual infração de trânsito, sendo certo que o arquivamento do auto é medida que se impõe. ', { align: 'left' });
  doc.moveDown();
...
doc.moveDown();
  ...
  doc.fontSize(15).font('Helvetica-Bold').text('IV.III - DA GARANTIA DO DIREITO DA AMPLA DEFESA E DO CONTRADITÓRIO', { align: 'left' });
  doc.moveDown();
  ...
  doc.fontSize(15).font('Helvetica-Bold').text('V – DOS PEDIDOS', { align: 'left' });
  doc.moveDown();
